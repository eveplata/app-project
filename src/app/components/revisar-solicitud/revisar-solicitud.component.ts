...
export class RevisarSolicitudComponent implements OnInit {
  ...
  @Output() productosSeleccionados: EventEmitter<ProductoSlt[]> =
    new EventEmitter();
    ...
    this.solicitud.productos.forEach((p) => {
      if (p.entregado === false) {
        this.productosService
          .getProductosPorId(p.id_producto)
          .subscribe((resp) => {
  ...
  checkboxClick(e: any, producto: ProductoSlt) {
    ...
    this.productosSeleccionados.emit(
      this.solicitud.productos.filter((p) => p.entregado)
    );
  }
